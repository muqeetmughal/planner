<template>
  <div class="dynamic-timeline min-h-screen bg-gray-50">
    <!-- Keep it clean and simple like roster -->
    ...
    <div v-else class="relative z-10 timeline-view mx-6 mb-6">
      <div
...
const loadTimelineData = async () => {
  ...
  try {
    ...
    // console.log('📊 Loading timeline data:', props.configuration.name, startDate, endDate, 'currentDate:', currentDate.value);
...
const handleBlockMove = async (data) => {
  try {
    // Handle datetime moves from day view
    let moveDate = data.newDate;
    if (data.newDateTime) {
      // Convert ISO datetime to date for the API
    ...
    const response = await call("planner.api.update_block_assignment", {
      ...
      new_date: moveDate,
      new_datetime: data.newDateTime, // Pass the full datetime for field updates
