<template>
  <div class="dynamic-timeline min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-blue-900/20 dark:to-indigo-900/20">
    ...
    <!-- Enhanced Header -->
    <div class="relative z-10 timeline-header mb-8 p-8 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 dark:border-gray-700/50 mx-6 mt-6">
      <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
        ...
        <div class="flex items-center gap-6">
          ...
          >
            <FeatherIcon name="arrow-left" class="w-4 h-4 group-hover:-translate-x-1 transition-transform" />
            <span class="font-medium">Back to Configurations</span>
          </Button>
          
          <div class="border-l-2 border-gradient-to-b from-blue-400 to-purple-400 pl-6">
            ...
            <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
              <span class="px-2 py-1 bg-blue-100 dark:bg-blue-900/30 rounded-md font-medium">{{ config?.row_doctype }}</span>
              <FeatherIcon name="arrow-right" class="w-3 h-3" />
              <span class="px-2 py-1 bg-purple-100 dark:bg-purple-900/30 rounded-md font-medium">{{ config?.block_doctype }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="date-navigation flex items-center gap-2 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl p-3 shadow-lg border border-gray-200/50 dark:border-gray-700/50">
            <Button 
              variant="ghost" 
              theme="gray" 
              size="sm"
            ...
            >
              <FeatherIcon name="chevron-left" class="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" />
            </Button>
            
            <div class="px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl text-white font-semibold min-w-[240px] text-center shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer group">
            ...
            <Button 
              ...
              class="group hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-xl transition-all duration-300"
            >
              <FeatherIcon name="chevron-right" class="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
            </Button>
          ...
          <div class="view-toggle flex bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm rounded-2xl p-2 shadow-lg border border-gray-200/50 dark:border-gray-700/50">
            <button 
            ...
            >
              ...
              <div v-if="currentViewMode === mode.value" class="absolute inset-0 bg-gradient-to-r from-blue-500 to-purple-600 animate-pulse"></div>
            </button>
          </div>
          ...
          >
            <FeatherIcon name="refresh-cw" :class="[
      ...
      <div class="mt-6 p-6 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-2xl border border-blue-200/50 dark:border-blue-700/50 backdrop-blur-sm">
        <div class="flex items-center justify-between text-sm">
          <div class="flex items-center gap-8">
            <div class="group flex items-center gap-3 hover:scale-105 transition-transform cursor-pointer">
              <div class="relative">
                <div class="w-4 h-4 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full shadow-lg"></div>
                <div class="absolute inset-0 bg-gradient-to-r from-blue-400 to-blue-500 rounded-full animate-ping opacity-20"></div>
              </div>
              <div>
                <div class="font-bold text-xl text-gray-800 dark:text-white">{{ rows.length }}</div>
                <div class="text-xs text-gray-600 dark:text-gray-400">{{ config?.row_doctype || 'Rows' }}</div>
              </div>
            </div>
            
            <div class="group flex items-center gap-3 hover:scale-105 transition-transform cursor-pointer">
              <div class="relative">
                <div class="w-4 h-4 bg-gradient-to-r from-green-500 to-emerald-600 rounded-full shadow-lg"></div>
                <div class="absolute inset-0 bg-gradient-to-r from-green-400 to-emerald-500 rounded-full animate-ping opacity-20"></div>
              </div>
              <div>
                <div class="font-bold text-xl text-gray-800 dark:text-white">{{ blocks.length }}</div>
                <div class="text-xs text-gray-600 dark:text-gray-400">{{ config?.block_doctype || 'Blocks' }}</div>
              </div>
            </div>
            
            <div class="group flex items-center gap-3 hover:scale-105 transition-transform cursor-pointer">
              <div class="relative">
                <div class="w-4 h-4 bg-gradient-to-r from-amber-500 to-orange-600 rounded-full shadow-lg"></div>
                <div class="absolute inset-0 bg-gradient-to-r from-amber-400 to-orange-500 rounded-full animate-ping opacity-20"></div>
              </div>
              <div>
                <div class="font-bold text-xl text-gray-800 dark:text-white">{{ unassignedBlocks.length }}</div>
                <div class="text-xs text-gray-600 dark:text-gray-400">Unassigned</div>
              </div>
            </div>
          </div>
    ...
    <div v-if="loading" class="relative z-10 flex items-center justify-center py-20">
      <div class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-2xl p-8 shadow-2xl border border-white/20 dark:border-gray-700/50">
        <div class="flex flex-col items-center gap-6">
          <div class="relative">
            <div class="w-12 h-12 border-4 border-blue-200 dark:border-blue-800 rounded-full"></div>
            <div class="absolute inset-0 w-12 h-12 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
          </div>
          <div class="text-center">
            <div class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Loading Timeline Data</div>
            <div class="text-sm text-gray-600 dark:text-gray-400">Please wait while we fetch your data...</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="error" class="relative z-10 mx-6">
      <div class="bg-gradient-to-r from-red-50 to-pink-50 dark:from-red-900/20 dark:to-pink-900/20 border border-red-200/50 dark:border-red-700/50 rounded-2xl p-8 backdrop-blur-sm shadow-lg">
        <div class="flex items-start gap-4">
          <div class="flex-shrink-0">
            <div class="w-12 h-12 bg-gradient-to-r from-red-500 to-pink-600 rounded-full flex items-center justify-center shadow-lg">
    ...
    <div v-else class="relative z-10 timeline-grid mx-6 mb-6">
      <div class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-gray-700/50 overflow-hidden shadow-2xl">
        <DynamicTimelineGrid
          :config="config"
          :rows="rows"
          :blocks="blocks"
    ...
    >
      ...
      >
        <div class="bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 dark:border-gray-700/50 w-full max-w-2xl max-h-[90vh] overflow-hidden">
          <div class="flex items-center justify-between p-6 border-b border-gray-200/50 dark:border-gray-700/50 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20">
    ...
    <div class="fixed bottom-6 right-6 z-40">
      <div class="relative group">
        ...
        >
          <div v-if="showQuickActions" class="absolute bottom-16 right-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 dark:border-gray-700/50 p-4 min-w-[200px]">
            <div class="space-y-2">
              <button class="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all duration-200 text-left">
                <FeatherIcon name="plus-circle" class="w-5 h-5 text-blue-500" />
                <span class="font-medium">Add Block</span>
              </button>
              <button class="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-green-50 dark:hover:bg-green-900/30 transition-all duration-200 text-left">
                <FeatherIcon name="download" class="w-5 h-5 text-green-500" />
                <span class="font-medium">Export</span>
              </button>
              <button class="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-purple-50 dark:hover:bg-purple-900/30 transition-all duration-200 text-left">
...
const loadTimelineData = async () => {
  ...
  try {
    ...
    const endDate = dateColumns.value[dateColumns.value.length - 1]?.date.toISOString().split('T')[0]
    
    const response = await call('planner.api.timeline_data.get_timeline_data', {
...
const handleBlockMove = async (data) => {
  try {
    const response = await call('planner.planner.api.timeline_data.update_block_assignment', {
