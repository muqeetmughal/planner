...
const processAssigneeData = (rawAssignees) => {
  ...
  const createTask = async (taskData) => {
    ...
    try {
      const resource = createResource({
        url: 'planner.api.create_atlas_task',
        params: {
          task_data: {
            ...taskData,
            exp_start_date: formatDateForAPI(taskData.exp_start_date),
            exp_end_date: formatDateForAPI(taskData.exp_end_date),
            _assign: taskData.assigned_to ? JSON.stringify([taskData.assigned_to]) : null
          }
        },
        onSuccess: (response) => {
          const newTask = processTaskData([response.task])[0]
